...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        forHis = np.histogram(formants,bins=10,range=(0,5500))
        #bins = np.bincount(forHis[0])
        return forHis[0]
    ...
    def _compute_pitch_features(self, window):
        ...
        pitHis = np.histogram(pitch_contour,bins=10,range=(0,128))

        mean = np.mean(pitch_contour)
        stand_dev = np.std(pitch_contour)
        #bins = np.bincount(pitHis[0])
        return np.append(pitHis[0],[mean,stand_dev])
    ...
    def _compute_delta_coefficients(self, window, n=2):
        ...
        denominator = 2 * np.sum(np.square(range(1, 3)))
        results = []
        for i in range(n, n_frames - n):
            buff = []
            for j in range(0, n_coeffs):
                temp1 = mfcc_feats[i+1][j]-mfcc_feats[i-1][j]
                temp2 = 2*(mfcc_feats[i+2][j]-mfcc_feats[i-2][j])
                buff.append((temp1+temp2)/denominator)
            results = np.append(results,buff)
        results = results.flatten()
        return results

        # TODO: implement delta coefficient calculation and return the result as 975-element 1D array

    #this will return a (71*13)*1 array since window size is 75*13 and drop 2 line at beginning and the end
    def _compute_delta_delta_coefficients(self, window, m=2):
        ddelta = self._compute_delta_coefficients(window, n=2)
        ddelta = np.reshape(ddelta, (75, 13))
        m_frames, m_coeffs = ddelta.shape
        denominator = 2 * np.sum(np.square(range(1, 3)))
        results = []
        for i in range(m, m_frames - m):
            buff = []
            for j in range(0, m_coeffs):
                temp1 = ddelta[i+1][j]-ddelta[i-1][j]
                temp2 = 2*(ddelta[i+2][j]-ddelta[i-2][j])
                buff.append((temp1+temp2)/denominator)
            results = np.append(results,buff)
        results = results.flatten()
        return results
