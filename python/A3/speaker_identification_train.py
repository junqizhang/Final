...
n_features = 997 # 10 formant features + 13 pitch contour features + 975 mfcc delta coefficients
...
for i, (train_indexes, test_indexes) in enumerate(cv):
   ...
   sys.stdout.flush()
   clf = RandomForestClassifier(n_estimators=50, max_features=500, max_depth=25)
   clf.fit(X_train, y_train)
...
best_classifier = RandomForestClassifier(n_estimators=50) # change this to the classifier (tested above) that gives the best results
